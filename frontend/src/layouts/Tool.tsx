/**
 * DISCLAIMER:
 * The code provided was developed hastily to focus on enhancing infrastructure skills,
 * not frontend development. It consists of a series of patches and does not reflect
 * my standard coding practices or real-life coding approach.
 */
...
const analyze = (
  ...
  analysisType: MutableRefObject<"erep" | "jurisprudence">,
  setResults: React.Dispatch<React.SetStateAction<Results | string[]>>,
  setLoading: React.Dispatch<
    React.SetStateAction<"idle" | "requesting" | "analyzing" | "noresults">
  >
) => {
  return async () => {
    ...
    const url =
      analysisType.current === "erep"
        ? "https://api.anoti.io/erep"
        : "https://api.anoti.io/jurisprudence";
...
function FakeLoader(props: {
  loading: "idle" | "requesting" | "analyzing" | "noresults";
}) {
  const { loading } = props;
  if (loading === "idle") return <></>;
  if (loading === "noresults")
    return (
      <p className="w-full text-center text-sm p-8">
        Pas de résultats trouvé, l'entreprise est clean!
      </p>
    );
  ...
  useEffect(() => {
    ...
    if (loading === "analyzing") {
      ...
      const timer4 = setTimeout(() => setProgress(90), 25000);
      return () => {
        clearTimeout(timer1);
        clearTimeout(timer2);
        clearTimeout(timer3);
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center p-8">
      ...
      {progress === 33 ? (
        <p className="m-4 text-xs">On récupère les données de l'entreprise</p>
      ) : (
        <></>
      )}
      {progress === 50 ? (
        <p className="m-4 text-xs">
          On cherche dans toutes les bases de données lié à l'entreprise
        </p>
      ) : (
        <></>
      )}
      ...
      {progress === 90 ? (
        <p className="m-4 text-xs">
          On filtres les documents, ça prend quelques secondes...
        </p>
      ) : (
        <></>
      )}
...
export function Tool() {
  ...
  let analysisType: MutableRefObject<"erep" | "jurisprudence"> =
    useRef("jurisprudence");
  const [testingTool, setTestingTool] = useState<boolean>(false);
  const [results, setResults] = useState<Results | string[]>([]);
  const [loading, setLoading] = useState<
    "idle" | "requesting" | "analyzing" | "noresults"
  >("idle");
  const expand = useSpring({
    maxHeight: results.length > 0 ? 700 : 400,
    config: { tension: 10, friction: 10 },
  });
  const openTool = useSpring({
    maxHeight: testingTool ? 2000 : 0,
    padding: testingTool ? 8 : 0,
    delay: 800,
    config: { tension: 10, friction: 10 },
  });
  const displayButtonTestTool = useSpring({
    maxHeight: !testingTool ? 400 : 0,
    padding: !testingTool ? 8 : 0,
  });
  const analyzerFn = analyze(
    companyName,
    risk,
    analysisType,
    setResults,
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center">
      <animated.div
        ...
        style={displayButtonTestTool}
      >
        <Button onClick={() => setTestingTool(true)}>
          Tester l'outil d'analyse de risque
        </Button>
      </animated.div>
      <animated.div style={openTool} className="w-9/12 p-8 overflow-hidden">
        <animated.div
          style={expand}
          className="rounded-md align-center bg-black tool-shadow animate-height overflow-auto"
        >
          <div className="w-full flex justify-between my-4 px-8">
            <div className="w-full flex gap-4">
              <Input
                onChange={(event) => (companyName.current = event.target.value)}
                className="w-1/2"
                placeholder="Rentrez un nom d'entreprise"
              ></Input>
              ...
              <Select
                onValueChange={(value) =>
                  (analysisType.current = value as "erep" | "jurisprudence")
                }
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    ...
                    <SelectItem value={"erep"}>erep</SelectItem>
                    <SelectItem value={"jurisprudence"}>
                      jurisprudence
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
            ...
            <Button
              disabled={loading === "analyzing" || loading === "requesting"}
              onClick={analyzerFn}
              variant="outline"
            >
              Lancer l'analyse
          ...
          {results.length > 0 ? <Results results={results}></Results> : <></>}
          {}
        </animated.div>
      </animated.div>
    </div>
...
function Results(props: { results: Results | string[] }): React.JSX.Element {
  const { results } = props;
  if (isArrayOfStrings(results)) {
    return (
